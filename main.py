...
def main():
    ...
    default_walking = normal_walking_sound
    ...
    while not playing:        
        ...
        # Declare variables
        startup_screen = interface.StartupScreen(window)
        startup_screen.draw()
        startup_screen.handle_events()

        if not startup_screen.run:
            break
    ...
    while playing:
        ...
        while True:
            ...
            # Check if the player is moving using keys
            if keys[K_a] == True or keys[K_d]== True or keys[K_s]== True or keys[K_w]== True:
                moving = True
            else:
                moving = False
            ...
            if moving:
                if sprinting:
                    if current_sound != "faster":
                        walking_channel.stop()
                        normal_walking_sound = pygame.mixer.Sound("sfx\\sounds\\faster_walking_sound.wav")
                        walking_channel.play(normal_walking_sound, -1)
                        current_sound = "faster"
                else:
                    if current_sound != "normal":
                        walking_channel.stop()
                        normal_walking_sound = default_walking
                        walking_channel.play(normal_walking_sound, -1)
                        current_sound = "normal"
            else:
                walking_channel.stop()
                current_sound = None

            # Main key events (Moving)
            if keys[K_w]:
                # Move UP
